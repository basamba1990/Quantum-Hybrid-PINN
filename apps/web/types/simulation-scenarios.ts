...
export const INDUSTRIAL_SCENARIOS: Record<ScenarioType, ScenarioConfig> = {
  ...
  DEEP_MINING_BLOCK: {
    ...
    name: 'Bloc minier profond',
    description: 'Analyse géomécanique du bloc minier profond à partir du contrat de cas, de la géométrie B-Rep, du maillage et des champs persistés.',
    inputs: [],
    outputs: []
  },
  MINING_INDUSTRIAL_SIM: {
...
export const SCENARIO_ALIASES: Record<string, ScenarioType> = {
  ...
  'DEEP MINING BLOCK': 'DEEP_MINING_BLOCK',
  'BLOC MINIER PROFOND': 'DEEP_MINING_BLOCK',
...
export function normalizeScenarioType(value?: string | null): ScenarioType | null {
  ...
  if (key.includes('DEEP MINING') || key.includes('DEEP_MINING') || key.includes('BLOC MINIER')) return 'DEEP_MINING_BLOCK';
