...
export type CfdArtifactViewState = {
  noArtifact: boolean;
...
/**
 * Keep the three backend states distinct:
 * - NO_CFD_ARTIFACT is an explicit business status;
 * - an error means the report could not be read;
 * - UNVALIDATED means an artefact exists but evidence is incomplete.
 */
export function getCfdArtifactViewState(
  analysisId: string | null | undefined,
  report: Pick<GateReport, "datasetPath" | "overallStatus" | "gates" | "error"> | null | undefined,
): CfdArtifactViewState {
  const noArtifact = !analysisId || report?.overallStatus === "NO_CFD_ARTIFACT";
  const reportError = !noArtifact && report?.error ? report.error : null;
  const artifactAvailable = Boolean(
    report?.datasetPath ||
    (report?.gates && report.gates.length > 0) ||
    (report?.overallStatus && report.overallStatus !== "NO_CFD_ARTIFACT"),
  );
  return { noArtifact, reportError, artifactAvailable };
}
...
export default function ScientificValidationWorkspace({
  ...
  const artifactViewState = useMemo(
    () => getCfdArtifactViewState(analysisId, gateReport),
    [analysisId, gateReport],
  );
  const { artifactAvailable, noArtifact, reportError } = artifactViewState;
  const statusLabel = {
    DRAFT: "DRAFT",
          ...
          <p className="mt-1 text-xs text-slate-500">
            {status === "VALIDATED"
              ...
              : noArtifact
                ...
                : reportError
                ? "Réessayez la lecture du rapport serveur"
                : "Sequential lock"}
          </p>
        </div>
      </div>
      ...
      <div className="mt-7 rounded-2xl border border-cyan-500/20 bg-cyan-500/[0.03] p-5">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <span className="font-mono text-[10px] uppercase tracking-widest text-cyan-300">
            {gateLoading
              ? "Reading server report…"
              : noArtifact
              ? "NO_CFD_ARTIFACT · audit unavailable"
              : reportError
              ? "G0–G5 REPORT UNAVAILABLE"
              : gateReport?.overallStatus || "Unavailable"}
          </span>
        </div>
        {noArtifact ? (
          ...
          <p className="text-xs text-amber-200">Erreur de lecture du rapport G0–G5 : {reportError}</p>
