...
export default function ScientificAuditCard({
  ...
  const sessionId = useMemo(() => auditData.analysisId || 'SYSTEM_VALIDATED', [auditData.analysisId]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white/[0.03] border border-white/10 rounded-3xl p-6 space-y-4">
            ...
            <div className="w-full bg-white/5 h-2 rounded-full overflow-hidden">
              <div className={`h-full transition-all duration-1000 ${score >= 90 ? 'bg-emerald-500' : 'bg-blue-500'}`} style={{ width: `${score}%` }} />
            </div>
          </div>
          ...
          <div className="bg-white/[0.03] border border-white/10 rounded-3xl p-6 space-y-4">
            ...
            <p className="text-[10px] text-gray-500 leading-relaxed font-medium">Analyse des résidus PDE effectuée sur le domaine volumétrique.</p>
          </div>
          ...
          <div className="bg-white/[0.03] border border-white/10 rounded-3xl p-6 space-y-4">
            ...
            <p className="text-[10px] text-gray-500 leading-relaxed font-medium">Incertitude statistique estimée par échantillonnage stochastique.</p>
          </div>
        </div>
          ...
          <div className="space-y-8">
            <div className="flex items-center justify-between px-2">
              ...
              <div className="flex bg-white/5 p-1 rounded-2xl border border-white/10">
                <button onClick={() => setVisualizationType('field')} className={`px-6 py-2 rounded-xl text-[10px] font-black transition-all ${visualizationType === 'field' ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}>CHAMP SCALAIRE</button>
                <button onClick={() => setVisualizationType('trajectory')} className={`px-6 py-2 rounded-xl text-[10px] font-black transition-all ${visualizationType === 'trajectory' ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-400 hover:text-white'}`}>TRAJECTOIRE</button>
              </div>
            </div>
          ...
          <div className="border border-red-500/20 rounded-[32px] overflow-hidden bg-red-500/[0.02]">
            <button onClick={() => setShowDetails(!showDetails)} className="w-full px-8 py-6 flex items-center justify-between text-red-400 hover:bg-red-500/[0.05] transition-all">
              <div className="flex items-center gap-4">
                ...
                <span className="text-sm font-black uppercase tracking-widest">{auditData.anomalies.length} Détection d'Anomalie Scientifique</span>
              </div>
