...
const facesForCell = (type: number, ids: number[]): Face[] => {
  switch (type) {
    case 5: return [[ids[0], ids[1], ids[2]]];
    case 9: return [[ids[0], ids[1], ids[2], ids[3]]];
...
export function buildCfdSurfaceMesh({ frame, fieldName, wireframe = false }: CFDMeshRendererProps): THREE.Mesh {
  ...
  for (let cell = 0; cell < frame.cellTypes.length; cell += 1) {
    ...
    if (frame.cellTypes[cell] === 5 || frame.cellTypes[cell] === 9) {
      surfaceFaces.push(...facesForCell(frame.cellTypes[cell], ids));
      continue;
    }
  ...
  const colors: number[] = [];
  const pushFace = (face: Face) => {
    const ids = face.filter((value): value is number => value !== undefined);
    if (new Set(ids).size < 3) return;
    const triangles = ids.length === 3 ? [[0, 1, 2]] : [[0, 1, 2], [0, 2, 3]];
    for (const triangle of triangles) triangle.forEach((index) => push(ids[index]));
  };
