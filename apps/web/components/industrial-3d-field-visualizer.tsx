...
interface Props {
  data?: DataPoint[]
  title?: string
  colorVariable?: 'temperature' | 'pressure' | 'velocity' | 'residual'
  maxPointsDisplay?: number
}

/**
 * VISUALISEUR 3D SCIENTIFIQUE V8.1 - CHAMPS SCALAIRES
 * - Affichage de champs 3D réels (pas de trajectoires synthétiques)
 * - Colorimétrie par variable physique avec légende précise
 * - Bounding box avec axes gradués
 * - Statistiques spatiales complètes
 * - Support des résidus physiques
 */
const Industrial3DFieldVisualizer: React.FC<Props> = ({ 
  data = [], 
  ...
  const [stats, setStats] = useState({ 
    minT: 0, maxT: 0, minP: 0, maxP: 0, minV: 0, maxV: 0, minR: 0, maxR: 0,
    count: 0, displayedCount: 0,
    xMin: 0, xMax: 12, yMin: -1, yMax: 1, zMin: 0, zMax: 12,
    meanValue: 0, stdValue: 0
  })
  ...
  useEffect(() => {
    ...
    const residuals = pointsToUse.map(p => p.residual_continuity || 0)

    const meanValue = temps.reduce((a, b) => a + b, 0) / temps.length
    const stdValue = Math.sqrt(temps.reduce((a, b) => a + (b - meanValue) ** 2, 0) / temps.length)
    ...
    const newStats = {
      ...
      minV: Math.min(...vels),
      maxV: Math.max(...vels),
      minR: Math.min(...residuals),
      maxR: Math.max(...residuals),
      count: data.length,
    ...
    const rRange = newStats.maxR - newStats.minR || 1
    ...
    pointsToUse.forEach((p, i) => {
      ...
      } else if (colorVariable === 'residual') {
        norm = ((p.residual_continuity || 0) - newStats.minR) / (rRange || 1)
      }

      norm = Math.max(0, Math.min(1, norm))

      // Dégradé scientifique: Bleu -> Cyan -> Jaune -> Rouge
  ...
  const getScaleInfo = () => {
    if (colorVariable === 'temperature') {
      return { min: stats.minT.toFixed(1), max: stats.maxT.toFixed(1), unit: 'K', mean: stats.meanValue.toFixed(1), std: stats.stdValue.toFixed(1) }
    } else if (colorVariable === 'pressure') {
      return { min: (stats.minP / 1e5).toFixed(2), max: (stats.maxP / 1e5).toFixed(2), unit: 'bar', mean: (stats.meanValue / 1e5).toFixed(2), std: (stats.stdValue / 1e5).toFixed(2) }
    } else if (colorVariable === 'velocity') {
      return { min: stats.minV.toFixed(1), max: stats.maxV.toFixed(1), unit: 'm/s', mean: stats.meanValue.toFixed(1), std: stats.stdValue.toFixed(1) }
    } else {
      return { min: stats.minR.toExponential(2), max: stats.maxR.toExponential(2), unit: 'Pa/s', mean: stats.meanValue.toExponential(2), std: stats.stdValue.toExponential(2) }
    }
  }
    ...
    <div className="w-full space-y-4">
      ...
      <div className="relative flex gap-4">
        ...
        <div className="w-24 flex flex-col">
          ...
          <div className="text-center text-[9px] text-gray-500 mt-2 space-y-1">
            <div className="font-bold">Max: {scaleInfo.max}</div>
            <div>Mean: {scaleInfo.mean}</div>
            <div>Std: {scaleInfo.std}</div>
            <div className="font-bold">Min: {scaleInfo.min}</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-5 gap-2 text-xs">
        ...
        <div className="bg-white/5 border border-white/10 rounded p-2">
          <div className="text-gray-400 text-[9px]">{colorVariable.toUpperCase()}</div>
          <div className="text-orange-400 font-mono text-[10px]">{scaleInfo.min} → {scaleInfo.max}</div>
        </div>
      </div>
    </div>
  )
...
export default Industrial3DFieldVisualizer
