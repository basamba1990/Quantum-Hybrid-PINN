...
def main():
    ...
    print(f"Validation de {len(projects)} projets industriels...")

    for proj in projects:
        pid = proj["id"]
        name = proj.get("name", "Industrial Project")
        scenario = proj.get("scenario_type", "")
        print(f"\nProjet: {name} ({pid})")

        is_sphere = "1250" in name or "1250" in scenario or "STORAGE" in scenario or "lh2" in name.lower() or "SPHERE" in name.upper()
        ...
        if analyses:
            ...
            # Create analysis
            new_an = supabase.table("analyses").insert({
                "project_id": pid,
                "name": name,
                "status": "completed",
